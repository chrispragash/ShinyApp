...
# Specify input$obs initially for debugging purposes.  Once this loads up on the server it is overwritten by the GUI.
  input = list(obs=27)
...
shinyServer(function(input, output) {
  ...
  output$summary <- renderPrint({
    ...
    #The following text formats the summary based on the percentile. A sample summary text reads like this (for item 69) "This item was taken by 78 respondents. This was a hard item.  Only 47% of people got it correct, putting it in the 73 percentile in terms of difficulty.  In terms of item parameter estimates, this item had a=0.94 and b=-0.08.  Having a small (a) estimate predicts the item to have difficulties discriminating between people who are above this level and those below."
